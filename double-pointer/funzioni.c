...
int inizializza_matrice (matrice *matrix,int righe, int colonne)
{
    ...
    matrix->valori = malloc(righe * sizeof(float*));
    for (i=0; i<righe; i++)
    {
        *(matrix->valori + i) = malloc(colonne * sizeof(float));
    }
    return 0;
}
      ...
      if ((fp = fopen(file,"rb"))!=NULL)
      {
          ...
          */
          ...
          else
          {
              ...
              {
                 //Adesso si leggono i valori
                 for (i=0; i<leggi_righe(*matrix) && !errore_fine_file;i++)
                 {
                     if (fread(*(matrix->valori + i),sizeof(float), leggi_colonne(*matrix),fp)!=leggi_colonne(*matrix))
                        errore_fine_file = 1;
...
int scrivi_su_file_bin (matrice * matrix, char * file)
{
      ...
      if ((fp = fopen(file,"ab"))!=NULL)
      {
          ...
          for (i=0; i<leggi_righe(*matrix);i++)
          {
              //Ora si scrivono i valori
              fwrite(*(matrix->valori + i),sizeof(float),leggi_colonne(*matrix),fp);
          }
          fclose(fp);
          return 0;
      }
